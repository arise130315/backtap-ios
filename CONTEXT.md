...
### 上架路线:国区优先(2026-05-28 决定)

**~~先海外、后国内~~** → **国区优先**。原决策(写于早期讨论)作废。

国区上架必备清单:

| 项 | 状态 | 备注 |
|---|---|---|
| 软著「快捷识屏翻译软件」V1.0 | ⏳ 代办中 | 详见下方决策条目 |
| 隐私政策 URL | ⏳ 待补 | 用户计划在 `backtap.cn` 官网补一页 |
| 工信部 App 备案 | ⏳ 软著 + 隐私政策齐备后办 | 2023.9 起强制,周期约 1-2 个月 |
| `PrivacyInfo.xcprivacy` Privacy Manifest | ❌ 未配 | 2024 起强制,海外国区都要 |
| 首次使用分析功能的同意弹窗 | ❌ 未做 | PIPL 要求,明示图片传给阿里云 |
| App 作为「服务方」独立备案 | ⚠️ 风险点 | 千问/DeepSeek 底层 API 已备案,但 App 套壳算不算需要独立备案,各地口径不一,办备案时再问 |

### 软著名称定为「快捷识屏翻译软件」V1.0

2026-05-28 确定,待办理。中国大陆 App Store 上架强制要求软著(2017 起苹果硬性规定)。

- **软著名**:`快捷识屏翻译软件` — 功能描述型 + 不带具体品牌词,后续 App Store 显示名怎么折腾(`快捷识屏` / `SnapTranslate` / `快捷识屏 Pro` 等)都能罩住,避免再办「变更登记」(¥150–300 / 10 个工作日)
- **版本号**:V1.0
- **申请人主体**:个人(必须和 Apple Developer 账号一致)
- **办理渠道**:倾向淘宝/代办公司,普通 ¥200–500、加急 ¥1500–3000;自办官费 ¥300,普通 30 个工作日
- **App 显示名约束**:子集/英文版/加后缀 OK,完全无关名字(如「AI 翻译大师」)会被驳回
